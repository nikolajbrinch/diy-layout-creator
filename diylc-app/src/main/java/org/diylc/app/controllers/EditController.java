...
public interface EditController extends MenuController {
  ...
  default void cut() {
    ...
    getClipboard().setContents(
        new ComponentTransferable(cloneComponents(getPlugInPort().getSelectedComponents())),
        getClipboardOwner());
  ...
  default void paste() {
    ...
    try {
      List<IDIYComponent> components =
          (List<IDIYComponent>) getClipboard().getData(ComponentTransferable.listFlavor);
  ...
  default void editProject() {
    ...
    PropertyEditorDialog editor =
        DialogFactory.getInstance().createPropertyEditorDialog(properties, "Edit Project");
  ...
  default void saveAsTemplate() {
    ...
    String templateName = JOptionPane.showInputDialog(null, "Template name:", "Save as Template",
