...
public class CanvasController extends AbstractController
    implements ArrangeController, EditController {

  public CanvasController(ApplicationController applicationController, View view, Model model,
  ...
  public void init(Project project, boolean freshStart) {
    getCanvas().refreshSize();
    int scrollX = project.getScrollX();
    int scrollY = project.getScrollY();
    ...
    if (freshStart) {
      ...
      Rectangle visibleRect = getCanvas().getVisibleRect();
      visibleRect.setLocation(scrollX,
          scrollY);
      getCanvas().scrollRectToVisible(visibleRect);
      getCanvas().revalidate();
    }
