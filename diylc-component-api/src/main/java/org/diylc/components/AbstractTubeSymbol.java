...
public abstract class AbstractTubeSymbol extends AbstractComponent {
    ...
    public void draw(GraphicsContext graphicsContext, ComponentState componentState, boolean outlineMode, Project project,
        ...
        if (outlineMode) {
            ...
            finalLabelColor = componentState == ComponentState.SELECTED || componentState == ComponentState.DRAGGING
                    ? Colors.LABEL_COLOR_SELECTED : Colors.LABEL_COLOR;
        }
        graphicsContext.setColor(finalLabelColor);
        Point p = getTextLocation();
        drawCenteredText(graphicsContext, display == Display.VALUE ? getValue() : getName(), new Point(p.x, p.y), HorizontalAlignment.LEFT,
