...
public abstract class AbstractLeadedComponent extends AbstractTransparentComponent {
    ...
    public void draw(GraphicsContext graphicsContext, ComponentState componentState, boolean outlineMode, Project project,
        ...
        if (shape == null) {
            ...
            graphicsContext.setColor(outlineMode ? Constants.TRANSPARENT_COLOR : getStandingBodyColor());
            ...
            if (outlineMode) {
                ...
                finalBorderColor = componentState == ComponentState.SELECTED || componentState == ComponentState.DRAGGING
                        ? Colors.SELECTION_COLOR : theme.getOutlineColor();
            } else {
                finalBorderColor = componentState == ComponentState.SELECTED || componentState == ComponentState.DRAGGING
            ...
            if (!outlineMode) {
                ...
                graphicsContext.setStroke(ObjectCache.getInstance().fetchBasicStroke(leadThickness));
                ...
                if (shouldShadeLeads()) {
                    graphicsContext.setStroke(ObjectCache.getInstance().fetchBasicStroke(leadThickness - 2));
                    leadColor = getLeadColorForPainting(componentState);
                    graphicsContext.setColor(leadColor);
                    graphicsContext.drawLine(points[0].x, points[0].y, (int) (points[0].x + Math.cos(theta) * leadLength),
                            (int) (points[0].y + Math.sin(theta) * leadLength));
                    graphicsContext.drawLine(points[1].x, points[1].y, (int) (points[1].x + Math.cos(theta - Math.PI) * leadLength),
            ...
            graphicsContext.translate((points[0].x + points[1].x - length) / 2, (points[0].y + points[1].y - width) / 2);
            ...
            if (outlineMode) {
                ...
                finalLabelColor = componentState == ComponentState.SELECTED || componentState == ComponentState.DRAGGING
                        ? Colors.LABEL_COLOR_SELECTED : theme.getOutlineColor();
            } else {
                finalLabelColor = componentState == ComponentState.SELECTED || componentState == ComponentState.DRAGGING
                        ? Colors.LABEL_COLOR_SELECTED : labelColor;
            }
            ...
            graphicsContext.drawString(label, (int) (length - textRect.getWidth()) / 2 + offset,
    ...
    private void drawLead(GraphicsContext graphicsContext, ComponentState componentState) {
        graphicsContext.setStroke(ObjectCache.getInstance().fetchBasicStroke(getLeadThickness()));
        Color leadColor = shouldShadeLeads() ? getLeadColorForPainting(componentState).darker() : getLeadColorForPainting(componentState);
        graphicsContext.setColor(leadColor);
        graphicsContext.drawLine(points[0].x, points[0].y, points[1].x, points[1].y);
        if (shouldShadeLeads()) {
            graphicsContext.setStroke(ObjectCache.getInstance().fetchBasicStroke(getLeadThickness() - 2));
            leadColor = getLeadColorForPainting(componentState);
            graphicsContext.setColor(leadColor);
            graphicsContext.drawLine(points[0].x, points[0].y, points[1].x, points[1].y);
        }
    ...
    protected Color getLeadColorForPainting(ComponentState componentState) {
        return componentState == ComponentState.SELECTED || componentState == ComponentState.DRAGGING ? Colors.SELECTION_COLOR
                : getLeadColor();
    }
