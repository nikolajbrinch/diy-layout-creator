...
public class SingleCoilPickup extends AbstractTransparentComponent<String> {
	...
	public void draw(GraphicsContext graphicsContext, ComponentState componentState, boolean outlineMode,
		...
		drawCenteredText(graphicsContext, value, bounds.x + bounds.width / 2, bounds.y + bounds.height / 2,
	...
	public void drawIcon(GraphicsContext graphicsContext, int width, int height) {
		...
		graphicsContext.fillRoundRect((width - bodyWidth) / 2, (height - bodyLength) / 2, bodyWidth,
				bodyLength, bodyWidth, bodyWidth);
		graphicsContext.fillPolygon(new int[] { width * 9 / 16, width * 9 / 16, width * 11 / 16, width * 11 / 16 },
				new int[] { (height - bodyLength) / 2, (height + bodyLength) / 2, height * 5 / 8,
						height * 3 / 8 }, 4);
		
		graphicsContext.setColor(Color.lightGray);
		graphicsContext.drawLine(width / 2, 4 * width / 32, width / 2, 4 * width / 32);
		graphicsContext.drawLine(width / 2, height - 4 * width / 32, width / 2, height - 4 * width / 32);
	}
